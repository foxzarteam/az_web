...
const SERVICES = [
  {
    ...
    icon: (
      <svg viewBox="0 0 48 48" fill="none" className="w-10 h-10 sm:w-12 sm:h-12 shrink-0" aria-hidden>
  ...
  return (
    <section className="bg-white dark:bg-darkmode relative overflow-hidden">
      <div className="container lg:max-w-screen-xl md:max-w-screen-md mx-auto px-4 sm:px-6 lg:px-8 relative z-10 max-w-full">
        <h2 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold mb-6 sm:mb-10 md:mb-12 text-midnight_text dark:text-white text-center" data-aos="fade-up">Our Services</h2>
        <div className="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6">
          {SERVICES.map((item, index) => (
            <div
              key={index}
              className={`relative rounded-xl border p-4 sm:p-5 md:p-6 flex flex-col min-h-[220px] sm:min-h-[260px] md:min-h-[280px] transition-colors duration-300 hover:shadow-md ${CARD_BGS[index]}`}
              data-aos="fade-up"
            ...
            >
              ...
              <div className="flex items-start gap-2 sm:gap-3 mb-3 sm:mb-4 pt-5 sm:pt-6">
                ...
                <h3 className="text-base sm:text-lg md:text-xl font-bold text-midnight_text dark:text-white pt-1">{item.title}</h3>
              </div>
