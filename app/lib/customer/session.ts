...
const REVOKED_COOKIE = "customer_session_revoked";
...
/** Local-only placeholder — never use in production. Set CUSTOMER_SESSION_SECRET in .env.local. */
const DEV_PLACEHOLDER_SECRET = "local-dev-customer-session-not-for-prod";
...
async function writeRevokedJtis(jtis: string[]) {
  const store = await cookies();
  store.set(REVOKED_COOKIE, signRevokedList(jtis.slice(0, MAX_REVOKED)), {
    httpOnly: true,
